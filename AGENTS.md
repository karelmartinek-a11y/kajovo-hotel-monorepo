...
- Správa uživatelů odděluje seznam od tříkrokového editoru. Přehled pokojů zachovává celé údaje a na mobilu na šířku nejméně dva pokoje v řádku; běžný svislý posuv je přípustný. Volba stavu otevře blokující průběh zápisu a po ověřené odpovědi automaticky vrátí přehled bez potvrzovacího tlačítka; neověřený zápis vyžaduje obnovu stavu před opakováním.
...
- Přihlášený portál i administrace používají sdílený `AppShell`: na desktopu stálý postranní panel, na tabletu a mobilu přichycené záhlaví s přímo dostupnými odkazy na povolené sekce a samostatným odkazem na profil. Navigace respektuje roli a oprávnění; úzký řádek odkazů se posouvá vodorovně, nikoli celý dokument.
