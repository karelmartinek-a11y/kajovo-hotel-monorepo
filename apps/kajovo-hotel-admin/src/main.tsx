...
const ADMIN_ROLE_VIEW_MODULES: Record<string, string[]> = {
  ...
  �dr�ba: ['issues'],
  sn�dan�: ['breakfast', 'issues', 'inventory'],
  sklad: ['breakfast', 'issues', 'inventory'],
};

const ADMIN_ROLE_VIEW_OPTIONS = ['admin', 'recepce', 'pokojsk�', '�dr�ba', 'sn�dan�', 'sklad'] as const;
...
function AppRoutes(): JSX.Element {
  ...
  const adminModules = auth.role === 'admin'
    ? [
      { key: 'users', label: 'U�ivatel�', route: '/uzivatele', icon: 'users', active: true, section: 'records', permissions: ['read'] },
  ...
  const allowedModules = orderedRoleModules.filter((module) => {
    // View-state odkazy jsou intern� QA trasa a v produk�n� navigaci nemaj� b�t vid�t.
    ...
    if (!required) {
      // Testovac� / injektovan� moduly bez explicitn�ch opr�vn�n� ukazujeme v�dy.
      return true;
    }
