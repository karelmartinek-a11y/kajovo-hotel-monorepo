...
type SmtpOperationalStatusReadModel = {
  ...
  delivery_mode: 'unconfigured' | 'mock' | 'smtp';
...
type AdminOverview = {
  breakfastSummary: BreakfastSummary | null;
  issues: Issue[];
...
const qaRuntimeEnabled = (import.meta as ImportMeta & {
  env?: {
    VITE_ENABLE_QA_RUNTIME?: string;
    VITE_QA_SERVICE_DATE?: string;
  };
}).env?.VITE_ENABLE_QA_RUNTIME === '1';

function resolveDefaultServiceDate(): string {
  const qaServiceDate = (import.meta as ImportMeta & {
    env?: { VITE_QA_SERVICE_DATE?: string };
  }).env?.VITE_QA_SERVICE_DATE;
  if (qaRuntimeEnabled && qaServiceDate && /^\d{4}-\d{2}-\d{2}$/.test(qaServiceDate)) {
    return qaServiceDate;
  }
...
const defaultServiceDate = resolveDefaultServiceDate();
...
function Dashboard(): JSX.Element {
  ...
  return (
    <main className="k-page" data-testid="dashboard-page">
      ...
      <h1>Přehled</h1>
      <StateSwitcher />
      {stateUI ? stateUI : loading || overview === null ? (
        <SkeletonPage />
      ) : error ? (
        <StateView
          title="Chyba"
          description={error}
          stateKey="error"
          action={<button className="k-button" type="button" onClick={load}>Obnovit</button>}
        />
      ) : (
        <>
          <div className="k-grid cards-3">
            <Card title="Snídaně dnes">
              <strong>{overview.breakfastSummary?.total_orders ?? 0}</strong>
              <p>{getSummaryCount(overview.breakfastSummary, 'pending')} čekajících objednávek</p>
            </Card>
            <Card title="Závady">
              <strong>{countOpenIssues(overview.issues)}</strong>
              <p>{countCriticalIssues(overview.issues)} kritických otevřených závad</p>
            </Card>
            <Card title="Sklad">
              <strong>{overview.lowStockItems.length}</strong>
              <p>{overview.lowStockItems.length === 0 ? 'Žádná položka není pod minimem' : 'položek pod minimem'}</p>
            </Card>
          </div>
          <div className="k-grid cards-2">
            <Card title="Hlášení">
              <strong>{countActiveReports(overview.reports)}</strong>
              <p>{overview.reports.filter((item) => item.status === 'open').length} otevřených hlášení</p>
              <Link className="k-nav-link" to="/hlaseni">Otevřít hlášení</Link>
            </Card>
            <Card title="Ztráty a nálezy">
              <strong>{countStoredLostFound(overview.lostFoundItems)}</strong>
              <p>{overview.lostFoundItems.filter((item) => item.status === 'claimed').length} čeká na předání</p>
              <Link className="k-nav-link" to="/ztraty-a-nalezy">Otevřít evidenci</Link>
            </Card>
          </div>
        </>
      )}
...
function BreakfastList(): JSX.Element {
  ...
  const stateMarker = <StateMarker state={state} />;
  const auth = useAuth();
  const canImport = isBreakfastManager(auth);
  const canReactivate = isBreakfastManager(auth);
  const canEditDiet = isBreakfastManager(auth);
  const canServe = isBreakfastOperator(auth);
...
function HousekeepingAdmin(): JSX.Element {
  ...
  const load = React.useCallback(() => {
    ...
    void loadAdminOverview(defaultServiceDate)
      ...
      .catch(() => setError('Nepodařilo se načíst operativní přehled pokojské.'))
      .finally(() => setLoading(false));
  }, [state]);
  ...
  return (
    <main className="k-page" data-testid="housekeeping-admin-page">
      ...
      {stateUI ? stateUI : loading || overview === null ? (
        ...
        <>
          <Card title="Operativní handoff">
            <p>
              Admin povrch nepřebírá denní zadávání pokojské. Slouží jako dohled nad živými frontami
              a rychlý přístup do navazujících modulů.
            </p>
          </Card>
          <div className="k-grid cards-3">
            <Card title="Snídaně dnes">
              <strong>{getSummaryCount(overview.breakfastSummary, 'pending')}</strong>
              <p>čekajících objednávek k přípravě</p>
              <Link className="k-nav-link" to="/snidane">Otevřít snídaně</Link>
            </Card>
            <Card title="Závady k řešení">
              <strong>{countOpenIssues(overview.issues)}</strong>
              <p>{countCriticalIssues(overview.issues)} kritických priorit</p>
              <Link className="k-nav-link" to="/zavady">Otevřít závady</Link>
            </Card>
            <Card title="Ztráty a nálezy">
              <strong>{countStoredLostFound(overview.lostFoundItems)}</strong>
              <p>položek v aktivní evidenci</p>
              <Link className="k-nav-link" to="/ztraty-a-nalezy">Otevřít evidenci</Link>
            </Card>
          </div>
          <div className="k-grid cards-2">
            <Card title="Sklad pod minimem">
              <strong>{overview.lowStockItems.length}</strong>
              <p>{overview.lowStockItems.map((item) => item.name).slice(0, 3).join(', ') || 'Žádné položky pod minimem'}</p>
              <Link className="k-nav-link" to="/sklad">Otevřít sklad</Link>
            </Card>
            <Card title="Aktivní hlášení">
              <strong>{countActiveReports(overview.reports)}</strong>
              <p>{overview.reports.filter((item) => item.status === 'in_progress').length} hlášení je v řešení</p>
              <Link className="k-nav-link" to="/hlaseni">Otevřít hlášení</Link>
            </Card>
          </div>
        </>
  ...
  return (
    <main className="k-page" data-testid="issues-list-page">
      ...
      {stateUI ? stateUI : error ? <StateView title="Chyba" description={error} stateKey="error" action={<button className="k-button" type="button" onClick={() => window.location.reload()}>Obnovit</button>} /> : items.length === 0 ? (
        <StateView title="Prázdný stav" description={isMaintenance ? 'Zatím nejsou otevřené závady.' : 'Zatím nejsou evidované žádné závady.'} stateKey="empty" action={canCreateIssue ? <Link className="k-button" to="/zavady/nova">Nahlásit závadu</Link> : undefined} />
      ) : (
        <>
          <div className="k-toolbar">
            ...
            {canCreateIssue ? <Link className="k-button" to="/zavady/nova">Nová závada</Link> : null}
          </div>
          <DataTable headers={['Název', 'Lokace', 'Pokoj', 'Priorita', 'Stav', 'Přiřazeno', 'Akce']} rows={items.map((item) => [
            ...
            <div className="k-toolbar" key={item.id}>
              <Link className="k-nav-link" to={`/zavady/${item.id}`}>Detail</Link>
              {canMarkDone && item.status !== 'resolved' && item.status !== 'closed' ? <button className="k-button" type="button" onClick={() => void markDone(item.id)}>Hotovo</button> : null}
            </div>,
          ])} />
        </>
      )}
...
function IssuesDetail(): JSX.Element {
  ...
  return (
    <main className="k-page" data-testid="issues-detail-page">
      ...
      {stateUI ? stateUI : error ? <StateView title="404" description={error} stateKey="404" action={<Link className="k-button secondary" to="/zavady">Zpět na seznam</Link>} /> : item ? <div className="k-card"><div className="k-toolbar"><Link className="k-nav-link" to="/zavady">Zpět na seznam</Link>{canEdit ? <Link className="k-button" to={`/zavady/${item.id}/edit`}>Upravit</Link> : null}{canMarkDone && item.status !== 'resolved' && item.status !== 'closed' ? <button className="k-button" type="button" onClick={() => void updateStatus('resolved')}>Hotovo</button> : null}{canReopen && (item.status === 'resolved' || item.status === 'closed') ? <button className="k-button secondary" type="button" onClick={() => void updateStatus('new')}>Znovu otevřít</button> : null}{canDelete ? <button className="k-button secondary" type="button" onClick={() => void fetchJson(`/api/v1/issues/${item.id}`, { method: 'DELETE' }).then(() => window.location.assign('/admin/zavady')).catch(() => setError('Smazání závady selhalo.'))}>Smazat</button> : null}</div><DataTable headers={['Položka', 'Hodnota']} rows={[[ 'Název', item.title],[ 'Lokace', item.location],[ 'Pokoj', item.room_number ?? '-'],[ 'Priorita', issuePriorityLabel(item.priority)],[ 'Stav', issueStatusLabel(item.status)],[ 'Přiřazeno', item.assignee ?? '-'],[ 'Popis', item.description ?? '-' ]]} /><h2>Timeline</h2><Timeline entries={timeline} />{photos.length > 0 ? <div className="k-grid cards-3">{photos.map((photo) => <img key={photo.id} src={`/api/v1/issues/${item.id}/photos/${photo.id}/thumb`} alt={`Fotografie závady ${photo.id}`} className="k-photo-thumb" />)}</div> : null}</div> : <SkeletonPage />}
    </main>
  );
}
...
function SettingsAdmin(): JSX.Element {
  ...
  const load = React.useCallback(() => {
    ...
    void Promise.all([
      ...
      fetchJson<SmtpSettingsReadModel>('/api/v1/admin/settings/smtp').catch((err: Error) => {
  ...
  async function sendTestEmail(): Promise<void> {
    ...
    setMessage(null);
    try {
      await fetchJson<{ ok: boolean; message: string }>('/api/v1/admin/settings/smtp/test-email', {
        method: 'POST',
...
function AppRoutes(): JSX.Element {
  ...
  const showFigure = panelLayout !== 'admin' && !['/intro', '/offline', '/maintenance', '/404'].includes(location.pathname);
...
<Route path="/pokojska" element={isAllowed('housekeeping') ? <HousekeepingAdmin /> : <AccessDeniedPage moduleLabel="Pokojská" role={auth.role} userId={auth.userId} />} />
        ...
        <Route path="/sklad" element={isAllowed('inventory') ? <InventoryWorkbench /> : <AccessDeniedPage moduleLabel="Skladové hospodářství" role={auth.role} userId={auth.userId} />} />
