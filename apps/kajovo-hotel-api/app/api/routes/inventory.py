...
DEFAULT_INVENTORY_ITEMS: list[InventoryItemCreate] = [
    InventoryItemCreate(name="Mouka", unit="g", min_stock=5000, current_stock=12000, amount_per_piece_base=1000),
    InventoryItemCreate(name="Cukr", unit="g", min_stock=3000, current_stock=8000, amount_per_piece_base=1000),
    InventoryItemCreate(name="Kava", unit="g", min_stock=2000, current_stock=4000, amount_per_piece_base=1000),
    InventoryItemCreate(name="Caj", unit="ks", min_stock=20, current_stock=80, amount_per_piece_base=1),
]
...
def seed_default_items(
    ...
    settings = get_settings()
    if not settings.inventory_seed_enabled:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Inventory bootstrap is disabled",
        )
