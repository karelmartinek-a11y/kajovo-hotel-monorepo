...
def upload_report_photos(
    ...
    if not report:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Report not found")
    if len(photos) > 3 or len(report.photos) + len(photos) > 3:
    ...
    return list(
        db.scalars(
            select(ReportPhoto).where(ReportPhoto.report_id == report.id).order_by(ReportPhoto.sort_order.asc())
        )
    )
...
def get_report_photo(
    ...
    photo = db.scalar(select(ReportPhoto).where(ReportPhoto.id == photo_id, ReportPhoto.report_id == report_id))
    if not photo:
